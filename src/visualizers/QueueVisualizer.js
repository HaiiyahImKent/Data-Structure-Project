...
export default function QueueVisualizer({ items, frontIndex, rearIndex }) {
    return (_jsx(motion.div, { className: "bg-slate-800/50 rounded-lg overflow-x-auto min-h-32", children: items.length === 0 ? (_jsx("div", { className: "flex h-full items-center justify-center p-6 text-slate-500", children: _jsx("p", { children: "Queue is empty" }) })) : (_jsx("div", { className: "flex items-center gap-4 px-6 py-4 min-w-max", children: items.map((item, index) => (_jsxs("div", { className: "flex flex-col items-center gap-2", children: [_jsx(motion.div, { layout: true, initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -20 }, className: `w-20 h-20 rounded-lg flex items-center justify-center font-bold text-white shadow-lg border-2 relative ${index === frontIndex
