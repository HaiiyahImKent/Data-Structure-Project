...
import { getStackPushSteps, getStackPopSteps, getQueueEnqueueSteps, getQueueDequeueSteps, getArrayInsertSteps, getArrayRemoveSteps, getLinkedListInsertSteps, getLinkedListRemoveSteps, getTreeInsertSteps, getHeapInsertSteps, } from "@utils/algorithmSteps";
import { AlertCircle, Plus, Trash2 } from "lucide-react";
const createLinkedListNodes = (values) => values.map((value, index) => ({ id: `ll-${Date.now()}-${index}`, value }));
...
const removeGraphNode = (data) => {
    ...
    return {
        data: {
            ...
            edges: data.edges.filter((edge) => edge.from !== removedIndex && edge.to !== removedIndex),
        },
...
const generateRandomHashEntries = () => Array.from({ length: 5 }, (_, index) => ({
    key: `key${index + 1}`,
...
export default function VisualizePage() {
    ...
    const [linkedListNodes, setLinkedListNodes] = useState(() => createLinkedListNodes(generateRandomArray(4, 50)));
    ...
    useEffect(() => {
        if (!visualization.isAnimating || visualization.algorithmSteps.length === 0)
            return;
        ...
        try {
            ...
            switch (structureId) {
                case "array":
                    ...
                    visualization.setAlgorithmSteps(getArrayInsertSteps(numericValue, arrayStore.items.length - 1, arrayStore.items));
                    mutated = true;
                    break;
                case "stack":
                    stackStore.push(numericValue);
                    visualization.setAlgorithmSteps(getStackPushSteps(numericValue, stackStore.items));
                    mutated = true;
                    break;
                case "queue":
                    queueStore.enqueue(numericValue);
                    visualization.setAlgorithmSteps(getQueueEnqueueSteps(numericValue, queueStore.items));
                    mutated = true;
                    break;
                case "linked-list":
                    setLinkedListNodes((prev) => {
                        const next = [...prev, { id: `ll-${Date.now()}`, value: numericValue }];
                        visualization.setAlgorithmSteps(getLinkedListInsertSteps(numericValue, next.length));
                ...
                case "heap":
                    setHeapItems((prev) => {
                        ...
                        visualization.setAlgorithmSteps(getHeapInsertSteps(numericValue, next.length));
                        return next;
                    });
                    mutated = true;
                    break;
        ...
        try {
            switch (structureId) {
                case "array": {
                    if (arrayStore.items.length > 0) {
                        ...
                        visualization.setAlgorithmSteps(getArrayRemoveSteps(arrayStore.items.length - 1, arrayStore.items));
                ...
                case "linked-list": {
                    setLinkedListNodes((prev) => {
                        ...
                        visualization.setAlgorithmSteps(getLinkedListRemoveSteps(prev[prev.length - 1].value, prev.length));
                        didMutate = true;
                ...
                case "tree": {
                    if (treeData) {
                        ...
                        const hasExplicitTarget = inputValue.trim().length > 0 && !Number.isNaN(numericInput);
                ...
                case "hash-table": {
                    const key = extractHashKey(inputValue) || hashTableStore.entries.slice(-1)[0]?.key;
                    if (key) {
                        ...
                        setInputError("Provide a key with key:value or in the input box to remove.");
                    }
                    break;
                }
                default:
            ...
            if (didMutate && removedValue !== undefined) {
                visualization.addOperation({
                    ...
                    value: removedValue,
                    timestamp: Date.now(),
                    complexity: calculateOperationComplexity(resolveOperationKey(structureId, "remove"), structureId),
                });
                setInputError(null);
                await sleep(500 * visualization.animationSpeed);
            }
    ...
    const handleRandomData = useCallback(() => {
        ...
        switch (structureId) {
            ...
            case "hash-table": {
                ...
                generateRandomHashEntries().forEach(({ key, value }) => hashTableStore.add(key, value));
                break;
            }
            default:
                break;
    ...
    const renderVisualizer = () => {
        ...
        switch (structureId) {
            ...
            case "queue":
                return (_jsx(QueueVisualizer, { items: queueStore.items, frontIndex: queueStore.frontIndex, rearIndex: queueStore.rearIndex }));
            case "linked-list":
                return (_jsx(LinkedListVisualizer, { nodes: linkedListNodes, highlightedIndex: highlightedLinkedIndex }));
            case "tree":
    ...
    return (_jsxs(motion.div, { initial: { opacity: 0 }, animate: { opacity: 1 }, className: "container mx-auto px-4 py-8", children: [_jsxs(motion.div, { initial: { opacity: 0, y: -20 }, animate: { opacity: 1, y: 0 }, className: "mb-8", children: [_jsxs("h1", { className: "text-4xl font-bold mb-2", children: [structure.icon, " ", structure.name, " Visualizer"] }), _jsxs("p", { className: "text-slate-400", children: ["Interactive playground for exploring ", structure.shortName] })] }), _jsxs("div", { className: "grid grid-cols-1 lg:grid-cols-3 gap-8", children: [_jsxs("div", { className: "lg:col-span-2 space-y-6", children: [_jsxs(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, className: "card-base", children: [_jsx("h2", { className: "text-xl font-bold mb-4", children: "Visualization" }), renderVisualizer()] }), _jsxs(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { delay: 0.1 }, className: "card-base space-y-4", children: [_jsx("h3", { className: "text-lg font-bold mb-4", children: "Controls" }), _jsxs("div", { className: "flex gap-2", children: [_jsx("input", { type: "text", value: inputValue, onChange: (e) => {
                                                    ...
                                                    setInputError(null);
                                                    setInputValue("");
                                                }, disabled: isAnimating, className: "btn-secondary py-2 flex items-center justify-center gap-1", children: [_jsx(Trash2, { size: 16 }), "Clear All"] }), _jsx(motion.button, { whileHover: { scale: 1.05 }, whileTap: { scale: 0.95 }, onClick: handleRandomData, disabled: isAnimating, className: "btn-secondary py-2 flex items-center justify-center gap-1", children: "\uD83C\uDFB2 Random" })] })] })] }), _jsxs("div", { className: "space-y-4", children: [_jsx(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { delay: 0.3 }, children: _jsx(PlaybackControls, { isAnimating: visualization.isAnimating, onPlay: () => visualization.setAnimating(true), onPause: () => visualization.setAnimating(false), onReset: () => {
                                        visualization.clearAlgorithmSteps();
                                        visualization.setAnimating(false);
                                    }, speed: visualization.animationSpeed, onSpeedChange: visualization.setAnimationSpeed, onUndo: visualization.undoLastOperation, onRedo: visualization.redoLastOperation, canUndo: visualization.historyIndex > 0, canRedo: visualization.historyIndex < visualization.history.length - 1, onPreviousStep: () => visualization.previousStep(), onNextStep: () => visualization.nextStep(), canPreviousStep: visualization.currentStepIndex > 0, canNextStep: visualization.currentStepIndex <
                                        visualization.algorithmSteps.length - 1, showStepControls: visualization.algorithmSteps.length > 0 }) }), _jsx(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { delay: 0.25 }, children: _jsx(ComplexityTable, { complexities: structure.complexity }) }), visualization.algorithmSteps.length > 0 && (_jsx(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { delay: 0.35 }, className: "card-base max-h-72", children: _jsx(AlgorithmStepsDisplay, { steps: visualization.algorithmSteps, currentStepIndex: visualization.currentStepIndex, isPlaying: visualization.isAnimating }) })), _jsx(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { delay: 0.4 }, className: "card-base max-h-64", children: _jsx(OperationLog, { logs: visualization.operations, onClear: visualization.clearOperations }) })] })] })] }));
}
