import { OperationLog } from "@store/visualizationStore";

export function calculateOperationComplexity(operation: string, dataStructure: string): string {
...
export function formatOperationLog(logs: OperationLog[]): string {
	return logs
		.map(
			(log) =>
				`[${new Date(log.timestamp).toLocaleTimeString()}] ${log.operation}${
					log.value !== undefined ? ` (${log.value})` : ""
				} - ${log.complexity || "Unknown"}`
		)
		.join("\n");
}

export function detectBottlenecks(logs: OperationLog[]): string[] {
	...
	const groupedByOp = logs.reduce((acc, log) => {
		if (!acc[log.operation]) acc[log.operation] = [];
...
export function getComplexityExplanation(complexity: string): string {
